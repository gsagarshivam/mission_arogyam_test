...
def index(request): 
    ...
    return render(request,'muscleveda/index.html',context)

def blogHome(request): 
    allBlog= Blog.objects.all()
    context={'allBlog': allBlog}
    return render(request, "muscleveda/blog/blogHome.html", context)

def blogPost(request, slug): 
    post=Blog.objects.filter(slug=slug).first()
    context={'post':post}
    return render(request, "muscleveda/blog/blogPost.html", context)
...
def contactus(request):
  if request.method == "POST":
      ...
      return render(request,"muscleveda/contactus.html")
...
def fileDownload(request): 
    ...
    return render(request,'muscleveda/download.html',context)

def video(request): 
    context={'file':Video.objects.all()}
    return render(request,'muscleveda/video.html',context)
