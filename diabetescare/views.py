...
def index(request): 
    ...
    return render(request,'diabetescare/index.html',context)

def blogHome(request): 
    allBlog= Blog.objects.all()
    context={'allBlog': allBlog}
    return render(request, "diabetescare/blog/blogHome.html", context)
...
def blogPost(request, slug): 
    ...
    context={'post':post}
    return render(request, "diabetescare/blog/blogPost.html", context)
...
def contactus(request):
  if request.method == "POST":
      ...
      return render(request,"diabetescare/contactus.html")
...
def diseaseHome(request): 
    ...
    return render(request, "diabetescare/diseases/diseaseHome.html", params)

def diseasePost(request, slug): 
    post=Disease.objects.filter(slug=slug).first()
    context={'post':post}
    return render(request, "diabetescare/diseases/diseasePost.html", context)

def fileDownload(request): 
    context={'file':FilesAdmin.objects.all()}
    return render(request,'diabetescare/download.html',context)

def video(request): 
    context={'file':Video.objects.all()}
    return render(request,'diabetescare/video.html',context)

def blogslug(request): 
    context={'file':Blog.objects.all()}
    return render(request,'diabetescare/base.html',context)

def eventHome(request):
    event = Event.objects.all()
    params = {'event':event}
    return render(request, "diabetescare/event/eventHome.html", params)

def eventPost(request, slug): 
    post=Event.objects.filter(slug=slug).first()
    context={'post':post}
    return render(request, "diabetescare/event/eventPost.html", context)
