...
def index(request): 
    ...
    return render(request,'heartcare/index.html',context)

def blogHome(request): 
    allBlog= Blog.objects.all()
    context={'allBlog': allBlog}
    return render(request, "heartcare/blog/blogHome.html", context)
...
def blogPost(request, slug): 
    ...
    context={'post':post}
    return render(request, "heartcare/blog/blogPost.html", context)
...
def contactus(request):
  if request.method == "POST":
      ...
      return render(request,"heartcare/contactus.html")
...
def diseaseHome(request): 
    ...
    return render(request, "heartcare/diseases/diseaseHome.html", params)

def diseasePost(request, slug): 
    post=Disease.objects.filter(slug=slug).first()
    context={'post':post}
    return render(request, "heartcare/diseases/diseasePost.html", context)

def fileDownload(request): 
    context={'file':FilesAdmin.objects.all()}
    return render(request,'heartcare/download.html',context)

def video(request): 
    context={'file':Video.objects.all()}
    return render(request,'heartcare/video.html',context)

def blogslug(request): 
    context={'file':Blog.objects.all()}
    return render(request,'heartcare/base.html',context)

def eventHome(request):
    event = Event.objects.all()
    params = {'event':event}
    return render(request, "heartcare/event/eventHome.html", params)

def eventPost(request, slug): 
    post=Event.objects.filter(slug=slug).first()
    context={'post':post}
    return render(request, "heartcare/event/eventPost.html", context)
