...
def index(request): 
    ...
    return render(request,'arogyaveda/index.html',context)

def blogHome(request): 
    allBlog= Blog.objects.all()
    context={'allBlog': allBlog}
    return render(request, "arogyaveda/blog/blogHome.html", context)

def blogPost(request, slug): 
    post=Blog.objects.filter(slug=slug).first()
    context={'post':post}
    return render(request, "arogyaveda/blog/blogPost.html", context)
...
def contactus(request):
  if request.method == "POST":
      ...
      return render(request,"arogyaveda/contactus.html")
...
def fileDownload(request): 
    ...
    return render(request,'arogyaveda/download.html',context)

def video(request): 
    context={'file':Video.objects.all()}
    return render(request,'arogyaveda/video.html',context)
